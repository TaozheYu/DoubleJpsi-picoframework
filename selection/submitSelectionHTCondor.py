...
parser.add_argument('-lp',   '--lnfPath',     dest='lnfPath',     action='store', type=str, default='/user/t/tayu/') #/cms/store/user/fromeo/')
parser.add_argument('-p',    '--process',     dest='process',     action='store', choices=['local','queue','crab'], default='queue')
parser.add_argument('-a',    '--analysis',    dest='analysis',    action='store', type=str, default='DoubleJpsi') #Name of the analysis (e.g. VBFHN, LQtop, ...)
...
parser.add_argument('-ys',   '--years',       dest='years',       action='store', type=str, default='2016') #The years you want to process (separated by _!) 
parser.add_argument('-dfd',  '--datasetFlagData', dest='datasetFlagData', action='store', type=str, default='Charmonium') #See datasets_info[d][1] in datasets_info.py. "All" stands for all he cases.
...
def main():
 ...
 while(keep_checking_count<iterations):
  ...
  os.system("cp /tmp/x509up_u93266 %sselection" % (path_fw)) 
   ...
   #Check/Merge
    ...
    if not numReadyRootFiles>0:
     if keep_checking_count==iterations: 
       #os.system("mail -s \"No files produced for %s %s\" fromeo@cern.ch < /dev/null" % (year,processName)) #If this happens there is a problem
    ...
    try:
     ...
     if keephadd:
      ...
      #if os.path.isfile(outputPathMergedFiles+"/"+processName+".root"): os.system("mail -s \"%s %s not deleted ;-(\" fromeo@cern.ch < /dev/null" % (year,processName))   
     elif keep_checking_count==iterations:
      incomplete_datasets = incomplete_datasets+1
      os.system("eos rm %s/%s.root" % (outputPathMergedFiles,processName)) 
      #if os.path.isfile(outputPathMergedFiles+"/"+processName+".root"): os.system("mail -s \"%s %s not deleted ;-(\" fromeo@cern.ch < /dev/null" % (year,processName))   
  ...
  #if not mergeOnly and keep_checking_count==1: os.system("mail -s \"%s submission finished on %s\" fromeo@cern.ch < /dev/null" % (task,hostname_))
 ...
 if datasetFlagData!='None':
  ...
  for y in theyears:
   ...
   os.popen('mv %sselection/*%s*.txt %sselection/%s/%s/txtFiles' % (path_fw,y,path_fw,analysis,task))
 ...
 #os.system("mail -s \"%s: Initial %s Finished %s (Readable %s Bad %s) Incomplete %s Null %s (%sx%s) data.root %s (%s) \" fromeo@cern.ch < /dev/null" % (task,num_datasets_of_interest,len(finished_datasetnames)+already_merged,good_datasets,bad_datasets,incomplete_datasets,null_datasets,keep_checking_count,timeSleep,dataproot_outcome,hostname)) 
...
def prepareSHFile(f,fileName,year,dataType,processName,lumiWeight):
 ...
 print >> subFile, "export X509_USER_PROXY=%s/x509up_u93266" % (path)
 print >> subFile, "voms-proxy-info -all"
 print >> subFile, "voms-proxy-info -all -file %s/x509up_u93266" % (path)
 if btagEff:
  ...
  print >> subFile, "python %s/search_runner_DoubleJpsi.py -p %s -y %s -dt %s -if %s" % (path,process,year,dataType,f)
 else:
