...
void RooFit_SPS_Jpsi(const char *type){
  gROOT->Reset();
  ...
  for(int i=0; i<name.size(); i++){
    ...
    sprintf(CUT,"");
    if (Type.find("official")!=string::npos) GetHisto(CUT, Tree_off, SPS ,plot,bin[i],Min[i],Max[i]);
    if (Type.find("private")!=string::npos) GetHisto(CUT, Tree_pri, SPS ,plot,bin[i],Min[i],Max[i]);
    ...
    canvas->SaveAs("figures/SPS_fit_"+NAME+"_"+Type+".pdf");
  }
}
