...
void RooFit_DPS_Jpsi(const char *type){
  gROOT->Reset();
  ...
  for(int i=0; i<name.size(); i++){
    ...
    sprintf(CUT,"");
    if (Type.find("official")!=string::npos) GetHisto(CUT, Tree_off, DPS ,plot,bin[i],Min[i],Max[i]);
    if (Type.find("private")!=string::npos) GetHisto(CUT, Tree_pri, DPS ,plot,bin[i],Min[i],Max[i]);
    ...
    canvas->SaveAs("figures/DPS_fit_"+NAME+"_"+Type+".pdf");
  }
}
