...
void RooFit3D_pseudo_data(){
  ...
  RooRealVar n_1("n_1","",10.2, 1,100);
  RooRealVar n_2("n_2","",29.19,1,100);
  ...
  RooRealVar DPS_mean("DPS_mean", "DPS mean", 0.000084417);
  ...
  RooRealVar DPS_n_1("DPS_n_1","",136, 1,200);
  RooRealVar DPS_n_2("DPS_n_2","",20.2, 1,100);
  ...
  RooGaussian DPS_gauss1("gauss1","gaussian1 PDF",z,DPS_mean1,DPS_width1);
  RooGaussian DPS_gauss2("gauss2","gaussian2 PDF",z,DPS_mean1,DPS_width2);
  ...
  RooRealVar gfrac1("gfrac1","",0.7,0.3,0.90);
  ...
  */
  ...
  RooProdPdf model_DCB_DPS("model_DCB_DPS","DCB_x*DCB_y*DPS_z",RooArgList(DCB_x,DCB_y,DPS_model));
  RooProdPdf model_DCB_BBbar("model_DCB_BBbar","DCB_x*DCB_y*BBbar_z",RooArgList(DCB_x,DCB_y,BBbar_DCB));
  RooAddPdf model_total1("model_total1","model_DCB_DPS+model_DCB_BBbar",RooArgList(model_DCB_DPS,model_DCB_BBbar),RooArgList(gfrac1));
  ...
  */
  TH1F* DPS = (TH1F*)data->Clone(); 
  TH1F* BBbar = (TH1F*)data->Clone(); 
  TH1F* Total = (TH1F*)data->Clone(); 
  ...
  double total_num = data_3D->Integral();
  ...
  */
  for(int k=0; k<name.size(); k++){
    if(k==0){
      ...
      model_total1.plotOn(xframe_data,LineColor(kGreen),LineWidth(1)); 
      ...
      xframe_data->SetMaximum(10000);;
      xframe_data->SetMinimum(0.1);;
      xframe_data->Draw();
      } 

    if(k==1){
      roohist_data_3D.plotOn(yframe_data);
      model_total1.plotOn(yframe_data,LineColor(kGreen),LineWidth(1)); 
      ...
      yframe_data->SetMaximum(10000);;
      yframe_data->SetMinimum(0.1);;
      yframe_data->Draw();
    }

    if(k==2){
      roohist_data_3D.plotOn(zframe_data);
      model_total1.plotOn(zframe_data,LineColor(kGreen),LineWidth(1)); 
      model_total1.plotOn(zframe_data,Components(model_DCB_DPS  ),LineColor(kBlue),LineWidth(1)); 
      model_total1.plotOn(zframe_data,Components(model_DCB_BBbar),LineColor(kRed),LineWidth(1)); 
      //model_total2.plotOn(yframe_data,LineColor(kRed),LineWidth(1)); 
      //model_total2.plotOn(yframe_data,Components(model_signal),FillStyle(3144),FillColor(kOrange),LineColor(kBlue),LineWidth(1)); 
      //model_total2.plotOn(yframe_data,Components(model_DCB1_comb2),LineStyle(9),LineColor(1),LineWidth(1)); 
      //model_total2.plotOn(yframe_data,Components(model_comb1_DCB2),LineStyle(7),LineColor(kViolet-2),LineWidth(1)); 
      ...
      zframe_data->GetYaxis()->SetTitle("Events/(0.01 GeV)"); 
      zframe_data->GetYaxis()->SetTitleOffset(1.30);
      zframe_data->SetMaximum(100000);;
      zframe_data->SetMinimum(0.1);;
      zframe_data->Draw();
    }
    ...
    TLegend *pl2; //= new TLegend(0.50,0.60,0.80,0.87);
    if(k==0) pl2 = new TLegend(0.30,0.20,0.60,0.45);
    if(k==1) pl2 = new TLegend(0.30,0.20,0.60,0.45);
    if(k==2) pl2 = new TLegend(0.50,0.60,0.80,0.87);
    pl2->SetTextSize(0.030);
    pl2->SetTextFont(62);
    pl2->SetFillColor(0);
    //TLegendEntry *ple2 = pl2->AddEntry(data, "Observed",  "PE");
    TLegendEntry *ple2 = pl2->AddEntry(data, "DPS and b#bar{b} #rightarrow J/#psi J/#psi mixed events",  "PE");
