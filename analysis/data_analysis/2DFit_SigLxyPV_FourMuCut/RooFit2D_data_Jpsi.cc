...
void RooFit2D_data_Jpsi(){
  ...
  TH2F *data_2D_SigLxyPV; //= new TH1F("DPS",  "DPS",  30,2.8,3.4);
  ...
  */
  ...
  RooRealVar gfrac2("gfrac2","",0.7, 0.6, 0.8);
  ...
  //name.push_back("fourMuFit_ups1_cTau_MC"); bin.push_back(200); Min.push_back(-0.02); Max.push_back(0.2); axis.push_back("J/#psi1 c#tau [cm]");
  name.push_back("Jpsi1_LxyPVSig"); bin.push_back(30); Min.push_back(0); Max.push_back(5); axis.push_back("Significance of L_{xy}(J/#psi1) [cm]");
  name.push_back("Jpsi2_LxyPVSig"); bin.push_back(30); Min.push_back(0); Max.push_back(5); axis.push_back("Significance of L_{xy}(J/#psi2) [cm]");
  ...
  RooProdPdf model_prompt_prompt("model_prompt_prompt","prompt_z*prompt_u",RooArgList(prompt_model_z,prompt_model_u));
  RooProdPdf model_prompt_nonprompt("model_prompt_nonprompt","prompt_z*nonprompt_u",RooArgList(prompt_model_z,non_prompt_model_u));
  RooProdPdf model_nonprompt_prompt("model_nonprompt_prompt","nonprompt_z*prompt_u",RooArgList(non_prompt_model_z,prompt_model_u));
  RooProdPdf model_nonprompt_nonprompt("model_nonprompt_nonprompt","nonprompt_z*nonprompt_u",RooArgList(non_prompt_model_z,non_prompt_model_u));
  ...
  for(int i=0; i<name.size(); i++){
   for(int j=i+1; j<name.size(); j++){
	...
	GetHisto(CUT, Tree, data ,plotX,bin[i],Min[i],Max[i]);
        ...
        GetHisto2D(CUT, Tree, data_2D_SigLxyPV ,plotZ,plotU,ZBIN,ZMIN,ZMAX,UBIN,UMIN,UMAX);
        //cout<<data_2D_mass->Integral()<<endl;
        cout<<data_2D_SigLxyPV ->Integral()<<endl;
  ...
  TH1F *h_prompt_prompt; h_prompt_prompt = (TH1F*)data->Clone(); h_prompt_prompt->SetLineColor(kBlue); h_prompt_prompt->SetLineWidth(1); 
  TH1F *h_prompt_nonprompt; h_prompt_nonprompt = (TH1F*)data->Clone();  h_prompt_nonprompt->SetLineColor(kGreen); h_prompt_nonprompt->SetLineWidth(1);
  TH1F *h_nonprompt_prompt; h_nonprompt_prompt = (TH1F*)data->Clone(); h_nonprompt_prompt->SetLineColor(kViolet-2); h_nonprompt_prompt->SetLineWidth(1);
  TH1F *h_nonprompt_nonprompt; h_nonprompt_nonprompt = (TH1F*)data->Clone(); h_nonprompt_nonprompt->SetLineColor(kOrange); h_nonprompt_nonprompt->SetLineWidth(1);  
  ...
  RooDataHist roohist_data_2D_SigLxyPV("roohist_data_SigLxyPV","roohist_data_SigLxyPV",RooArgSet(z,u),data_2D_SigLxyPV);
  ...
  //2D+2DD fit
  model_SigLxyPV.fitTo(roohist_data_2D_SigLxyPV,SumW2Error(kTRUE));
  Double_t frac2 = gfrac2.getVal();
  //Double_t frac1_err = gfrac1.getError(); 
  //Double_t frac2_err = gfrac2.getError(); 
  cout<<"prompt fraction is : "<<frac2<<endl; 
  //parameter (g2+(1-g2))*(g2+(1-g2))
  Double_t frac11 = frac2*frac2;
  Double_t frac12 = frac2*(1-frac2); 
  Double_t frac21 = (1-frac2)*frac2; 
  Double_t frac22 = (1-frac2)*(1-frac2); 
  ...
  RooAddPdf model_SigLxyPV_v2("","", RooArgList(model_prompt_prompt, model_prompt_nonprompt, model_nonprompt_prompt, model_nonprompt_nonprompt),
   RooArgList(gfrac11, gfrac12, gfrac21));
  
    
  for(int k=0; k<name.size(); k++){
    
    if(k==2){
      roohist_data_2D_SigLxyPV.plotOn(zframe_data);
      model_SigLxyPV_v2.plotOn(zframe_data,LineColor(kRed),LineWidth(1)); 
 
      model_SigLxyPV_v2.plotOn(zframe_data,Components(model_prompt_prompt),LineColor(kBlue),LineWidth(1)); 

      model_SigLxyPV_v2.plotOn(zframe_data,Components(model_prompt_nonprompt),LineColor(kGreen),LineWidth(1)); 

      model_SigLxyPV_v2.plotOn(zframe_data,Components(model_nonprompt_prompt),LineColor(kViolet-2),LineWidth(1)); 

      model_SigLxyPV_v2.plotOn(zframe_data,Components(model_nonprompt_nonprompt),LineColor(kOrange),LineWidth(1)); 
    ...
    if(k==3){
      roohist_data_2D_SigLxyPV.plotOn(uframe_data);
      model_SigLxyPV_v2.plotOn(uframe_data,LineColor(kRed),LineWidth(1)); 
 
      model_SigLxyPV_v2.plotOn(uframe_data,Components(model_prompt_prompt),LineColor(kBlue),LineWidth(1)); 

      model_SigLxyPV_v2.plotOn(uframe_data,Components(model_prompt_nonprompt),LineColor(kGreen),LineWidth(1)); 

      model_SigLxyPV_v2.plotOn(uframe_data,Components(model_nonprompt_prompt),LineColor(kViolet-2),LineWidth(1)); 

      model_SigLxyPV_v2.plotOn(uframe_data,Components(model_nonprompt_nonprompt),LineColor(kOrange),LineWidth(1)); 
    ...
    TLegend *pl2 = new TLegend(0.50,0.55,0.85,0.87);
    pl2->SetTextSize(0.030);
    pl2->SetTextFont(62);
    pl2->SetFillColor(0);
    TLegendEntry *ple2 = pl2->AddEntry(data, "Observed",  "PE");
    pl2->AddEntry(h_total, "total fit model", "L");
    pl2->AddEntry(h_prompt_prompt, " prompt + prompt", "L");
    pl2->AddEntry(h_prompt_nonprompt, "prompt + nonprompt", "L");
    pl2->AddEntry(h_nonprompt_prompt, "nonprompt + prompt", "L");
    pl2->AddEntry(h_nonprompt_nonprompt, "nonprompt + nonprompt", "L");
    pl2->SetBorderSize(0);
    pl2->Draw();

    TString NAME = name[k];
    cout<<"Finished "<<NAME<<endl;
    canvas->SaveAs("data_"+NAME+".pdf");
  }
...
void GetHisto(char CUT[1000], TTree *Tree, TH1F* & histo, const char *plot, int BIN, float MIN, float MAX){
  char input[1000]; sprintf(input,"%s>>h(%i,%f,%f)",plot,BIN,MIN,MAX);
