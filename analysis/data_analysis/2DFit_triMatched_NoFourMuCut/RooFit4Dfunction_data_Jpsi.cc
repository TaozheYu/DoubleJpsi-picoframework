...
void RooFit4Dfunction_data_Jpsi(){
  gROOT->Reset();
  ...
  float Jpsi_mass_do = 3.0;
  float Jpsi_mass_up = 3.2;

  RooRealVar x("x", "x", 2.80, 3.40);
  RooRealVar y("y", "y", 2.80, 3.40);
  RooRealVar z("z", "z", -0.2, 1);
  RooRealVar u("u", "u", -0.2, 1);
  ...
  RooRealVar HigherLimit("HigherLimit", "HigherLimit", 1);
  ...
  //Non-prompt and prompt pdf
    //prompt fit
  RooRealVar prompt_mean1("prompt_mean1", " mean1", 0.00007376); 
  ...
  */
  ...
  RooRealVar BBbar_Tau("BBbar_Tau", "BBbar_Tau", -2.63);      //-26.3
  ...
  RooAbsPdf* Exp_BBbar_u = new RooGenericPdf("BBbar_u", "BBbar_u", "Step_u * exp(u * BBbar_Tau)", RooArgSet(*Step_u, u, BBbar_Tau));
    ...
    //Gauss
  RooRealVar BBbar_Mean("BBbar_Mean", "BBbar_Mean",0.0171);  //0.00171
  RooRealVar BBbar_Sigma("BBbar_Sigma", "BBbar_Sigma", 0.0235); //0.00235
  ...
  RooRealVar gfrac_prompt("gfrac_prompt","",0.518);
  ...
  */
  ...
  RooRealVar gfrac_prompt_prompt("gfrac_prompt_prompt","gfrac_prompt_prompt", 0.25);  
  RooRealVar gfrac_nonprompt_prompt("gfrac_nonprompt_prompt","gfrac_nonprompt_prompt", 0.25, 0.2, 0.3);  
  RooRealVar gfrac_nonprompt_nonprompt("gfrac_nonprompt_nonprompt","gfrac_nonprompt_nonprompt", 0.25);  
  RooAddPdf model_ctau2("model_ctau2","model_ctau2", RooArgList(model_nonprompt_nonprompt, model_prompt_nonprompt, model_nonprompt_prompt, model_prompt_prompt),
   RooArgList(gfrac_nonprompt_nonprompt, gfrac_nonprompt_prompt, gfrac_nonprompt_prompt));
  /*
  ...
  */
  ...
  model_ctau.fitTo(*data_ctau_sim);//,SumW2Error(kTRUE)
  //model_ctau.fitTo(roohist_data_2D_ctau);//,SumW2Error(kTRUE)
  //roohist_data_2D_ctau.plotOn(zframe_data);
  data_ctau_sim->plotOn(zframe_data);
  //model_ctau.plotOn(zframe_data, Normalization(12475, RooAbsReal::Relative), LineColor(kRed), LineWidth(1)); 
  model_ctau.plotOn(zframe_data, LineColor(kRed), LineWidth(1)); 
  zframe_data->Draw();
