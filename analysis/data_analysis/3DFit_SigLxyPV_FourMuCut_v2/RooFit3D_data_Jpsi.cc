...
void RooFit3D_data_Jpsi(){
  ...
  RooRealVar Jpsi_width("Jpsi_width", "Jpsi width",0.029985);
  RooRealVar alpha_1("alpha_1", "",1.29976);
  RooRealVar alpha_2("alpha_2", "",1.28188);
  RooRealVar n_1("n_1","",4.025);
  RooRealVar n_2("n_2","",69.99);
  ...
  RooRealVar a0("a0","a0",-0.00653);
  RooRealVar a1("a1","a1",-0.09214);
  ...
  */
   //SPS Exponential
     //Generic Exponential 
   RooRealVar SPS_Tau("SPS_Tau", "SPS_Tau", -1.437);      //1
   RooAbsPdf* Expx_SPS = new RooGenericPdf("SPS", "SPS", "Stepx * exp(z * SPS_Tau)", RooArgSet(*Stepx, z, SPS_Tau));
   //Gauss
   RooRealVar SPS_Mean("SPS_Mean", "SPS_Mean",0.5999, 0.55, 0.8 );
   RooRealVar SPS_Sigma("SPS_Sigma", "SPS_Sigma", 0.31);
  ...
  */
    ...
    //Generic Exponential
  RooRealVar BBbar_Tau("BBbar_Tau", "BBbar_Tau", -1.7969);      //1

  RooAbsPdf* Expx_BBbar = new RooGenericPdf("BBbar", "BBbar", "Stepx * exp(z * BBbar_Tau)", RooArgSet(*Stepx, z, BBbar_Tau));

    //Gauss
  RooRealVar BBbar_Mean("BBbar_Mean", "BBbar_Mean",1.73495);
  RooRealVar BBbar_Sigma("BBbar_Sigma", "BBbar_Sigma", 0.804);
  ...
  //name.push_back("fourMuFit_ups1_cTau_MC"); bin.push_back(200); Min.push_back(-0.02); Max.push_back(0.2); axis.push_back("J/#psi1 c#tau [cm]");
  name.push_back("MaxLxyPVSig"); bin.push_back(100); Min.push_back(0); Max.push_back(5); axis.push_back("Max significance of L_{xy}(J/#psi) [cm]");
  ...
  for(int k=0; k<name.size(); k++){
    ...
    TLegend *pl2 = new TLegend(0.55,0.50,0.75,0.87);
    pl2->SetTextSize(0.025);
    pl2->SetTextFont(62);
    pl2->SetFillColor(0);
    TLegendEntry *ple2 = pl2->AddEntry(data, "Observed",  "PE");
    pl2->AddEntry(h_total, "total fit model", "L");
    pl2->AddEntry(h_DCB_DCB_prompt, "J/#psi1+J/#psi2 Max SigL_{xy}PV prompt", "L");
    pl2->AddEntry(h_DCB_comb_prompt, "J/#psi1+comb Max SigL_{xy}PV prompt", "L");
    pl2->AddEntry(h_comb_DCB_prompt, "comb.+J/#psi2 Max SigL_{xy}PV prompt", "L");  
    pl2->AddEntry(h_comb_comb_prompt, "comb.+comb. Max SigL_{xy}PV prompt", "L");  
    pl2->AddEntry(h_DCB_DCB_nonprompt, "J/#psi1+J/#psi2 Max SigL_{xy}PV non-prompt", "L");
    pl2->AddEntry(h_DCB_comb_nonprompt, "J/#psi1+comb Max SigL_{xy}PV non-prompt", "L");
    pl2->AddEntry(h_comb_DCB_nonprompt, "comb.+J/#psi2 Max SigL_{xy}PV non-prompt", "L");  
    pl2->AddEntry(h_comb_comb_nonprompt, "comb.+comb. Max SigL_{xy}PV non-prompt", "L");  
