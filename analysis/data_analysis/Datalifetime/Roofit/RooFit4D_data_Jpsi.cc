...
void RooFit4D_data_Jpsi(){
  ...
  */
   ...
   RooAbsPdf* Exp_SPS_u = new RooGenericPdf("SPS", "SPS", "Step_u * exp(u * SPS_Tau)", RooArgSet(*Step_u, u, SPS_Tau));
     ...
     //Gauss
   RooRealVar SPS_Mean("SPS_Mean", "SPS_Mean",0.169, 0.16, 0.33);
   RooRealVar SPS_Sigma("SPS_Sigma", "SPS_Sigma", 0.22215, 0 , 0.5);
   RooRealVar SPS_Sigma2("SPS_Sigma2", "SPS_Sigma2", 0.3333, 0 , 0.5);
   //RooRealVar SPS_Mean("SPS_Mean", "SPS_Mean",0.2769);
   //RooRealVar SPS_Sigma("SPS_Sigma", "SPS_Sigma", 0.190);
   RooAbsPdf* Gauss_SPS_z = new RooGaussian("Gauss_SPS", "Gauss_SPS", z, SPS_Mean, SPS_Sigma);
   RooAbsPdf* Gauss2_SPS_z = new RooGaussian("Gauss2_SPS", "Gauss2_SPS", z, SPS_Mean, SPS_Sigma2);
   RooRealVar gfrac_gauss1("gfrac_gauss1","",0.5, 0, 1);
   RooAddPdf Double_gauss_model("Double_gauss_model","Double_gauss_model",RooArgList(*Gauss_SPS_z, *Gauss2_SPS_z),RooArgList(gfrac_gauss1));
   ...
   RooFFTConvPdf prompt_model_z("ConvPdf_SPS_z", "ConvPdf_SPS_z", z, *Exp_SPS_z, Double_gauss_model);
  ...
  */
  ...
  //RooFFTConvPdf non_prompt_model_u("ConvPdf_BBbar_u", "ConvPdf_BBbar_u", u, *Exp_BBbar_u, *Gauss_BBbar_u);
    ...
    //UFO Gauss process
   RooRealVar UFO_Mean("UFO_Mean", "UFO_Mean",0.85);
   RooRealVar UFO_Sigma("UFO_Sigma", "UFO_Sigma",0.05, 0.06, 0.1);
   RooAbsPdf* Gauss_UFO_z = new RooGaussian("Gauss_UFO", "Gauss_UFO", z, UFO_Mean, UFO_Sigma);

   RooRealVar UFO2_Mean("UFO2_Mean", "UFO2_Mean",1.25);
   RooRealVar UFO2_Sigma("UFO2_Sigma", "UFO2_Sigma", 0.2, 0.05, 1);
   RooAbsPdf* Gauss_UFO2_z = new RooGaussian("Gauss_UFO2", "Gauss_UFO2", z, UFO2_Mean, UFO2_Sigma);

  //RooRealVar gfrac2("gfrac2","",0.7, 0.4, 0.9);
  //RooRealVar gfrac_SPS("gfrac_SPS","",0.0001);
  RooRealVar gfrac_prompt("gfrac_prompt","",0.8, 0.7, 0.9);
  RooRealVar gfrac_nonprompt("gfrac_nonprompt","",0.0884, 0.07, 0.15);
  RooRealVar gfrac_ufo("gfrac_ufo","",0, 0.012);
  ...
  //RooAddPdf model_SigLxyPV("model_SigLxyPV","model_SigLxyPV",RooArgList(prompt_model_z, non_prompt_model_z, *Gauss_UFO_z),RooArgList(gfrac_prompt ,gfrac_nonprompt));
  //RooAddPdf model_SigLxyPV("model_SigLxyPV","model_SigLxyPV",RooArgList(prompt_model_z, non_prompt_model_z, *Gauss_UFO_z, *Gauss_UFO2_z),RooArgList(gfrac_prompt ,gfrac_nonprompt,  gfrac_ufo));
  ...
  TH1F *h_UFO; h_UFO = (TH1F*)data->Clone(); h_UFO->SetLineColor(kOrange); h_UFO->SetLineWidth(1);
  TH1F *h_UFO2; h_UFO2 = (TH1F*)data->Clone(); h_UFO2->SetLineColor(kViolet); h_UFO2->SetLineWidth(1);
  ...
  */
  ...
  model_SigLxyPV.plotOn(zframe_data,Components(*Gauss_UFO_z),LineColor(kOrange),LineWidth(1)); 
  model_SigLxyPV.plotOn(zframe_data,Components(*Gauss_UFO2_z),LineColor(kViolet),LineWidth(1)); 
  ...
  pl2->AddEntry(h_UFO, "UFO1 process", "L");
  pl2->AddEntry(h_UFO2, "UFO2 process", "L");
  pl2->SetBorderSize(0);
  pl2->Draw();
