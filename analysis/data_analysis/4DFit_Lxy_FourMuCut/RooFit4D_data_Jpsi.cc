...
void RooFit4D_data_Jpsi(){
  ...
  TH2F *data_2D_Lxy; //= new TH1F("DPS",  "DPS",  30,2.8,3.4);
  ...
  RooRealVar z("z", "z", 0, 0.1);
  RooRealVar u("u", "u", 0, 0.1);
  ...
  */
  ...
  //name.push_back("fourMuFit_ups1_cTau_MC"); bin.push_back(200); Min.push_back(-0.02); Max.push_back(0.2); axis.push_back("J/#psi1 c#tau [cm]");
  name.push_back("Jpsi1_Lxy"); bin.push_back(40); Min.push_back(0.); Max.push_back(0.06); axis.push_back("ctau_{J/#psi1} [cm]");
  name.push_back("Jpsi2_Lxy"); bin.push_back(40); Min.push_back(0.); Max.push_back(0.06); axis.push_back("ctau_{J/#psi2} [cm]");
  ...
  for(int i=0; i<name.size(); i++){
   for(int j=i+1; j<name.size(); j++){
	...
	GetHisto(CUT, Tree, data ,plotX,bin[i],Min[i],Max[i]);
        ...
        GetHisto2D(CUT, Tree, data_2D_Lxy ,plotZ,plotU,ZBIN,ZMIN,ZMAX,UBIN,UMIN,UMAX);
        cout<<data_2D_mass->Integral()<<endl;
        cout<<data_2D_Lxy ->Integral()<<endl;
	...
	for(int p=1; p<data_2D_Lxy->GetNbinsX()+1; p++){
          for(int q=1; q<data_2D_Lxy->GetNbinsY()+1; q++){
              data_2D_Lxy->SetBinContent(p,q,data_2D_Lxy->GetBinContent(p,q));
              data_2D_Lxy->SetBinError(p,q,sqrt(data_2D_Lxy->GetBinContent(p,q)));
  ...
  RooDataHist roohist_data_2D_Lxy("roohist_data_SigLxyPV","roohist_data_SigLxyPV",RooArgSet(z,u),data_2D_Lxy);
  ...
  //data_2D_Lxy->Draw("LEGO");
  //data_2D_Lxy->Draw("COLZ");
  //canvas->SaveAs("data2D_ctau.pdf");
  data_z->Draw("");
  //data_z->Draw("COLZ");
  canvas->SaveAs("data_Lxy.pdf");
  //2D+2DD fit
  model_total1.fitTo(roohist_data_2D_Lxy,SumW2Error(kTRUE));
  ...
  model_total2.fitTo(roohist_data_2D_Lxy,SumW2Error(kTRUE));
  ...
  for(int k=0; k<name.size(); k++){
    ...
    if(k==2){
      roohist_data_2D_Lxy.plotOn(zframe_data);
    ...
    if(k==3){
      roohist_data_2D_Lxy.plotOn(uframe_data);
