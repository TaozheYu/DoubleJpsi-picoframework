...
void RooFit2D_sideband_data_Jpsi(){
  gROOT->Reset();
  ...
  float Jpsi_mass_SR_do = 2.9;
  float Jpsi_mass_SR_up = 3.3;
  ...
  RooRealVar x("x", "x", 2.70, 3.50);
  RooRealVar y("y", "y", 2.70, 3.50);
  x.setBins(1000,"cache");
  x.setRange("R1",2.7,2.9);
  x.setRange("R2",3.3,3.5);
  ...
  //RooRealVar Jpsi_mean_y("Jpsi_mean_y", "Jpsi mean", 3.092); 
  //RooRealVar Jpsi_width_y("Jpsi_width_y", "Jpsi width",0.03,0.01,0.1);
  //RooRealVar alpha_1_y("alpha_1_y", "",1,0,10);
  //RooRealVar alpha_2_y("alpha_2_y", "",1,0,10);
  //RooRealVar n_1_y("n_1_y","",10,0,70);
  //RooRealVar n_2_y("n_2_y","",10,0,70);
  ...
  RooRealVar a0("a0","a0", -0.2809, -0.3, -0.25);
  RooRealVar a1("a1","a1", 0.0324);
  ...
  name.push_back("Jpsi1_mass"); bin.push_back(60); Min.push_back(2.7); Max.push_back(3.5); axis.push_back("m_{12}[GeV]");
  name.push_back("Jpsi2_mass"); bin.push_back(60); Min.push_back(2.7); Max.push_back(3.5); axis.push_back("m_{34}[GeV]");
  ...
  for(int i=0; i<name.size(); i++){
    for(int j=i+1; j<name.size(); j++){
      ...
      char CUT[1000];
      //sprintf(CUTpre,"v_mumufit_VtxCL_noMC>%f && fourMuFit_VtxProb>%f && fourMuFit_ups1_mass>%f && fourMuFit_ups1_mass<%f && fourMuFit_ups2_mass>%f && fourMuFit_ups2_mass<%f",twomuon_cut, fourmuon_cut, Jpsi_mass_do, Jpsi_mass_up, Jpsi_mass_do, Jpsi_mass_up );
      //sprintf(CUT_sideband,"(v_mumufit_VtxCL_noMC>%f && fourMuFit_VtxProb>%f && ((fourMuFit_ups1_mass>%f && fourMuFit_ups1_mass<%f) || (fourMuFit_ups1_mass>%f && fourMuFit_ups1_mass<%f)) && ((fourMuFit_ups2_mass>%f && fourMuFit_ups2_mass<%f) || (fourMuFit_ups2_mass>%f && fourMuFit_ups2_mass<%f)))",twomuon_cut, fourmuon_cut, Jpsi_mass_CR_do, Jpsi_mass_SR_do, Jpsi_mass_SR_up, Jpsi_mass_CR_up, Jpsi_mass_CR_do, Jpsi_mass_SR_do, Jpsi_mass_SR_up, Jpsi_mass_CR_up );
      sprintf(CUT_sideband,"(trigger_matched && ((Jpsi1_mass>%f && Jpsi1_mass<%f) || (Jpsi1_mass>%f && Jpsi1_mass<%f)))", Jpsi_mass_CR_do, Jpsi_mass_SR_do, Jpsi_mass_SR_up, Jpsi_mass_CR_up );
      sprintf(CUT,"1 *%s ", CUT_sideband);
  ...
  //2D fit
  //model_comb1_comb2.fitTo(roohist_data_2D,SumW2Error(kTRUE));
  /*
  ...
  */
  for(int k=0; k<name.size(); k++){
    if(k==0){
      comb_x.fitTo(roohist_data_x,Range("R1,R2"),SumW2Error(kTRUE));
      roohist_data_x.plotOn(xframe_data);
      comb_x.plotOn(xframe_data,Range("R1,R2"),LineColor(1),LineWidth(1)); 
    ...
    if(k==1){
      ...
      model_comb1_comb2.plotOn(yframe_data,LineColor(1),LineWidth(1)); 
    ...
    */
    ...
    TLegend *pl2 = new TLegend(0.60,0.65,0.88,0.87);
