  ...
  */
  ...
  for(int i=0; i<name.size(); i++){
   for(int j=i+1; j<name.size(); j++){
	...
	for(int p=1; p<data_2D_SigLxyPV->GetNbinsX()+1; p++){
          for(int q=1; q<data_2D_SigLxyPV->GetNbinsY()+1; q++){
              data_2D_SigLxyPV->SetBinContent(p,q,data_2D_SigLxyPV->GetBinContent(p,q));
              data_2D_SigLxyPV->SetBinError(p,q,sqrt(data_2D_SigLxyPV->GetBinContent(p,q)));
  ...
  //2D+2DD fit
  model_total1.fitTo(roohist_data_2D_SigLxyPV,SumW2Error(kTRUE));
  ...
  model_total2.fitTo(roohist_data_2D_SigLxyPV,SumW2Error(kTRUE));
