...
void RooFit2D_fDPS_data(const char *CutCategory, float min_cut, float max_cut){
  gROOT->Reset();
  ...
  string CutCategory_=CutCategory;
  ...
  //for(int nbin=0; nbin<N; nbin++){
    //float min = mass_OFbins[nbin];
    //float max = mass_OFbins[nbin+1];
    float min = min_cut;
    float max = max_cut;
    ...
    for(int i=0; i<name.size()-1; i++){
      ...
      const char *plotY = name[i+1];
      int XBIN = bin[i];  int YBIN = bin[i+1];
      float XMIN = Min[i]; float YMIN = Min[i+1];
      float XMAX = Max[i]; float YMAX = Max[i+1];
      char CUTpre[1000];
      char CUT[1000];
      ...
      GetHisto(CUT, Tree, data_y ,plotY,bin[i+1],Min[i+1],Max[i+1]);
