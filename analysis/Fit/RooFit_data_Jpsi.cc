...
void RooFit_data_Jpsi(){
  ...
  RooRealVar a0("a0","a0",0.01,-100,100);
  RooRealVar a1("a1","a1",0.01,-100,100);
  RooChebychev comb("comb","comb",x,RooArgList(a0,a1));

  //construct composite pdf
  RooRealVar gfrac("gfrac","",0.2,0,1.0);
  RooAddPdf model("model","model",RooArgList(DCB,comb),RooArgList(gfrac));
  ...
  for(int i=0; i<name.size(); i++){
    ...
    model.fitTo(roohist_data,SumW2Error(kTRUE));
    ...
    model.plotOn(frame_data,Components(DCB),FillColor(kRed),LineColor(kBlue),LineWidth(1)); 
    frame_data->SetTitle(""); 
    ...
    TLegend *pl2 = new TLegend(0.55,0.60,0.88,0.87);
    pl2->SetTextSize(0.030);
    pl2->SetTextFont(62);
    pl2->SetFillColor(0);
    TLegendEntry *ple2 = pl2->AddEntry(&roohist_data, "Observed",  "PE");
    pl2->AddEntry(h_DCB, "Jpsi+Jpsi", "L");
    pl2->AddEntry(h_comb, "Comb.+comb.", "F");
