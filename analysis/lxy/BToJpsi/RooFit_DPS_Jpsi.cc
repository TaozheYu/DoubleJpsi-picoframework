...
void RooFit_DPS_Jpsi(){
  ...
  name.push_back("MuMuFit_ctauPV_MC"); bin.push_back(300); Min.push_back(-0.02); Max.push_back(0.2); axis.push_back("J/#psi c#tau [cm]");
  ...
  for(int i=0; i<name.size(); i++){
    ...
    CtauModel.fitTo(roohist_DPS,SumW2Error(kTRUE));
    roohist_DPS.plotOn(frame_DPS);
    CtauModel.plotOn(frame_DPS,LineColor(kBlue)); 
    frame_DPS->SetTitle(""); 
    ...
    pl2->SetTextFont(62);
    pl2->SetFillColor(0);
    TLegendEntry *ple2 = pl2->AddEntry(&roohist_DPS, "B to Jpsi",  "PE");
