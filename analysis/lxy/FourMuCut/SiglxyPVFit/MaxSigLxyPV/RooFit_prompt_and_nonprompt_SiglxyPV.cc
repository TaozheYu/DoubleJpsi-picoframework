...
void RooFit_prompt_and_nonprompt_SiglxyPV(){
  ...
  //DPS fit function parameter
    //DPS Exponential
    //Generic Exponential 
  RooRealVar DPS_Tau("DPS_Tau", "DPS_Tau", -1.317, -2.5, -0.5);      //1

  RooAbsPdf* Expx_DPS = new RooGenericPdf("DPS", "DPS", "Stepx * exp(x * DPS_Tau)", RooArgSet(*Stepx, x, DPS_Tau));
  
    //Gauss
  RooRealVar DPS_Mean("DPS_Mean", "DPS_Mean",0.1693, 0.1, 1);
  RooRealVar DPS_Sigma("DPS_Sigma", "DPS_Sigma", 0.22525, 0.1, 1);
  RooAbsPdf* Gauss_DPS = new RooGaussian("Gauss_DPS", "Gauss_DPS", x, DPS_Mean, DPS_Sigma);

    //Convolution
  RooFFTConvPdf ConvPdf_DPS("ConvPdf_DPS", "ConvPdf_DPS", x, *Expx_DPS, *Gauss_DPS);

  //SPS fit

    //SPS Exponential
    //Generic Exponential 
  RooRealVar SPS_Tau("SPS_Tau", "SPS_Tau", -1.317, -2.5, -0.5);      //1

  RooAbsPdf* Expx_SPS = new RooGenericPdf("SPS", "SPS", "Stepx * exp(x * SPS_Tau)", RooArgSet(*Stepx, x, SPS_Tau));
  
    //Gauss
  RooRealVar SPS_Mean("SPS_Mean", "SPS_Mean",0.1693, 0.1, 1);
  RooRealVar SPS_Sigma("SPS_Sigma", "SPS_Sigma", 0.22525, 0.1, 1);
  ...
  //BBbarToJpsiJpsi fit function parameter
    //Generic Exponential 
  RooRealVar BBbar_Tau("BBbar_Tau", "BBbar_Tau", -0.8418, -2, -0.3);      //1

  RooAbsPdf* Expx_BBbar = new RooGenericPdf("BBbar", "BBbar", "Stepx * exp(x * BBbar_Tau)", RooArgSet(*Stepx, x, BBbar_Tau));
  
    //Gauss
  RooRealVar BBbar_Mean("BBbar_Mean", "BBbar_Mean",0.5374, 0.1, 2);
  RooRealVar BBbar_Sigma("BBbar_Sigma", "BBbar_Sigma", 0.5993, 0.1, 2);
  ...
  for(int i=0; i<name.size(); i++){
    ...
    //sprintf(CUT,"");
    sprintf(CUT,"fourMu_VtxProb>0.01");
    ...
    DPS->SetLineColor(kGreen); DPS->SetLineWidth(2); DPS->SetMarkerColor(kGreen); DPS->SetMarkerSize(1.3); DPS->SetMarkerStyle(20);
    ...
    ConvPdf_DPS.fitTo(roohist_DPS,SumW2Error(kTRUE));
    roohist_DPS.plotOn(frame_SPS,LineColor(kGreen),MarkerColor(kGreen));
    ConvPdf_DPS.plotOn(frame_SPS,LineColor(kGreen)); 

    ConvPdf_SPS.fitTo(roohist_SPS,SumW2Error(kTRUE));
    roohist_SPS.plotOn(frame_SPS,LineColor(kBlue),MarkerColor(kBlue));
    //roohist_SPS.plotOn(frame_SPS,LineColor(1),MarkerColor(1));
    ...
    pl2->AddEntry(DPS, "DPS simulation events",  "PE");
