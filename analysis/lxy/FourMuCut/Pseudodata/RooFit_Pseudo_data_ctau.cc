...
void RooFit_Pseudo_data_ctau(){
  ...
  RooRealVar BBbar_n_2("BBbar_n_2","",8.7886);
  ...
  RooRealVar gfrac1("gfrac1","",0.9,0.70,0.95);
  ...
  for(int i=0; i<name.size(); i++){
    ...
    */
    ...
    DPS = (TH1F*)Pseudo_data->Clone(); 
    BBbar = (TH1F*)Pseudo_data->Clone(); 
    Total = (TH1F*)Pseudo_data->Clone(); 
    ...
    //RooDataHist roohist_Pseudo("roohist_Pseudo","roohist_Pseudo",x,Pseudo_data);
    //RooDataHist roohist_Pseudo("roohist_Pseudo","roohist_Pseudo",x,Pseudo_data_BBbarDouble_DPSHalf);
    RooDataHist roohist_Pseudo("roohist_Pseudo","roohist_Pseudo",x,Pseudo_data_BBbarHalf_DPSDouble);
    
    //double total_num = Pseudo_data->Integral();
    //double total_num = Pseudo_data_BBbarDouble_DPSHalf->Integral();
    double total_num = Pseudo_data_BBbarHalf_DPSDouble->Integral();

    RooPlot* frame_DPS = x.frame();

    CtauModel.fitTo(roohist_Pseudo,SumW2Error(kTRUE));
    roohist_Pseudo.plotOn(frame_DPS);
    //CtauModel.plotOn(frame_DPS,Components(DPS_DCB),LineColor(kBlue)); 
    CtauModel.plotOn(frame_DPS,Components(DPS_model),LineColor(kBlue)); 
    CtauModel.plotOn(frame_DPS,Components(BBbar_DCB),LineColor(kRed)); 
    CtauModel.plotOn(frame_DPS,LineColor(kGreen)); 
    ...
    pl2->SetTextFont(62);
    pl2->SetFillColor(0);
    TLegendEntry *ple2 = pl2->AddEntry(&roohist_Pseudo, "DPS and b#bar{b} #rightarrow J/#psi J/#psi mixed events",  "PE");
